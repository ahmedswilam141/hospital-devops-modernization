...
<div id="apDiv12"><hr>
  <div id="apDiv15">
  	<?php
		...
		$uid=$_SESSION['id'];
		$qry=mysqli_query($con, "select pimage from patient where patientID='$uid'");
  ...
  <?php
 	include("connection.php");
	$uid=$_SESSION['id'];
	$qry=mysqli_query($con, "select pname from patient where patientID='$uid'");
	while($row=mysqli_fetch_array($qry))
	{
     ...
     <th height="44">Patient Name</th>
      ...
      <tr>
       <?php
		include("connection.php");
		$uid=$_SESSION['id'];
		$qry1=mysqli_query($con, "select * from patient where patientID='$uid'");
		while($row=mysqli_fetch_array($qry1))
		{
		$qry2=mysqli_query($con, "select * from doctor natural join appointment where doctor.doctorID=appointment.doctorID");
		if($qry2)
		{
			while($doctor=mysqli_fetch_assoc($qry2))
			{
				if($uid==$doctor['patientID'])
				{
					...
					$did=$doctor['doctorID'];
					print "<input type='hidden' name='h2' value='$did'>";
